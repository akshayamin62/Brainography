...
export default function SuperAdminPaymentHistoryPage() {
  ...
  const handleDownload = async (paymentId: string, studentName: string) => {
    try {
  ...
  const handleSend = async (paymentId: string) => {
    ...
    try {
      await paymentAPI.sendInvoice(paymentId);
      toast.success('Invoice sent to student email');
    } catch {
                      ...
                      <td className="px-4 py-3">
                        {p.status === 'paid' && (
                          <div className="flex gap-2">
                            <button
                              onClick={() => handleDownload(p._id, getStudentName(p))}
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50"
                            >
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                              </svg>
                              {sendingId === p._id ? 'Sending...' : 'Send'}
                            </button>
                          </div>
                        )}
                      </td>
